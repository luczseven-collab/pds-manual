...
/**
 * リトライ付きAPIフェッチ。529/500/overloadedエラー時に指数バックオフで再試行する。
 * @param {string} url
...
function _fetchWithRetry(url, options, maxRetries) {
  ...
  const retryableCodes = [429, 500, 529];
  let lastResponse;
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    lastResponse = UrlFetchApp.fetch(url, Object.assign({ muteHttpExceptions: true }, options));
    const code = lastResponse.getResponseCode();
    if (!retryableCodes.includes(code)) return lastResponse;
    if (attempt < maxRetries) {
      const waitMs = Math.pow(2, attempt) * 3000; // 3s, 6s, 12s
      Logger.log('Claude API ' + code + ' エラー。' + (waitMs / 1000) + '秒後にリトライ (' + (attempt + 1) + '/' + maxRetries + ')');
...
JSONのみを返してください。説明文・コードブロックは不要です。`;
    ...
    const response = _fetchWithRetry(CLAUDE_API_URL, {
      method: 'post',
      headers: {
    ...
    if (httpCode !== 200) throw new Error('Claude API エラー (' + httpCode + '): ' + response.getContentText());
